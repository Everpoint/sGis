 ...
 */
export class Snapping extends Control {
    private _tempLayer: FeatureLayer;
    private _snapping: any;
    ...
    symbol = new PointSymbol({fillColor: 'red', size: 5});
    ...
    getSnapping(point) {
        ...
        for (let i = 0; i < this.snappingTypes.length; i++) {
...
let snapping = {
        ...
        for (let i = 0; i < features.length; i++) {
            ...
            for (let ringIndex = 0; ringIndex < rings.length; ringIndex++) {
                ...
                for (let j = 1; j < ring.length; j++) {
                    ...
                    let projection = pointToLineProjection(point.position, [ring[j-1], ring[j]]);
    ...
    orthogonal: function(point, layer, distance, activeFeature, activeRing = null, activeIndex = null) {
        ...
        for (let i = 0; i < lines.length; i++) {
            let projection = pointToLineProjection(point.position, lines[i]);
            ...
            if (Math.abs(dx) < distance && Math.abs(dy) < distance) {
                ...
                if (nextPoint && prevPoint) {
                    projection = pointToLineProjection(prevPoint, [ring[activeIndex], nextPoint]);
                    if (Math.abs(projection[0] - point.x) < distance && Math.abs(projection[1] - point.y) < distance) {
